...
func (t *udp) findnode(toid NodeID, toaddr *net.UDPAddr, target NodeID) ([]*Node, error) {
	...
	errc := t.pending(toid, neighborsPacket, func(r interface{}) bool {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				log.Debug(fmt.Sprintf("udp.findnode(): Invalid neighbor node received, ip = %v, addr = %v, err = %v", rn.IP, toaddr, err))
				continue
			}
			log.Debug(fmt.Sprintf("[udp]: findnode() --> callback func. node = %v:%v|%v \n\n", n.IP, n.UDP, n.TCP))
