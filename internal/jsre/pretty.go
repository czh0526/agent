...
func (ctx ppctx) printValue(v otto.Value, level int, inArray bool) {
	...
	case v.IsNull():
		fmt.Fprintf(ctx.w, "null")
	case v.IsUndefined():
		fmt.Fprintf(ctx.w, "undefined")
	case v.IsString():
		s, _ := v.ToString()
		fmt.Fprintf(ctx.w, "%q", s)
	case v.IsBoolean():
		b, _ := v.ToBoolean()
		fmt.Fprintf(ctx.w, "%t", b)
	case v.IsNaN():
		fmt.Fprintf(ctx.w, "NaN")
	case v.IsNumber():
		s, _ := v.ToString()
		fmt.Fprintf(ctx.w, "%s", s)
	default:
		fmt.Fprintf(ctx.w, "<unprintable>")
	}
}
...
func (ctx ppctx) printObject(obj *otto.Object, level int, inArray bool) {
	...
	case "Object":
		...
		if ctx.isBigNumber(obj) {
			fmt.Fprint(ctx.w, "%s", toString(obj))
			return
		}
